...
static void ShotDragon(sEnemyShotSet* pEnemyShotSet)
{
    ...
    while (pEnemyShot != pEnemyShotSet->pEnemyShotHead) {
        ...
        if (pEnemyShot->kind == img_enemyShotBullet[3]) {
            pEnemyShot->muki += sin(pEnemyShot->count * 0.1) * 0.01;
        }
...
void EnemyPat_Dragon_Qwen()
{
    ...
    if (count == 1) {
        ...
        if (enemy.y >= 250) if (enemy_y_add >= -0.3) enemy_y_add -= 0.1;
        if (enemy.y <= 150) if (enemy_y_add <= 0.3) enemy_y_add += 0.1;
    }
    ...
    if (count % 600 == 30) {
        ...
        pEnemyShotSet->y = enemy.y * 0 + 20.0; // 敵の少し下（口元）から発射
